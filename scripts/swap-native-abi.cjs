 ...
 *  - Electron (ABI 128) para rodar a aplicação
 *
 * Uso:
...
function getCurrentAbi() {
  try {
    ...
    if (e.message.includes('NODE_MODULE_VERSION 128')) return 'electron';
...
switch (mode) {
  case 'electron':
    cache('electron');
    restore('electron');
    console.log('[swap] ✅ Ready for Electron (ABI 128)');
    break;
  ...
  case 'status': {
    ...
    console.log('[swap] Run `node scripts/swap-native-abi.cjs electron` to swap to Electron (ABI 128)');
