        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          {['DINHEIRO', 'PIX', 'DÉBITO', 'CRÉDITO'].map(m => (
            <button 
              ...
              className={`py-2 rounded-lg border font-bold text-[10px] transition-all ${paymentMethod === m ? 'bg-emerald-600 border-emerald-400 text-slate-950' : 'bg-surface-light border-border text-muted'}`}
            >
              {m}
            </button>
          ))}
