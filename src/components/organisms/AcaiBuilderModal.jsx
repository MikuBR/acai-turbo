      ...
      <div className="bg-card w-full max-w-5xl h-[85vh] rounded-2xl border border-border flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="flex-1 overflow-hidden flex gap-6 p-6">
          <div className="w-80 flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
            <section>
              <h3 className="text-[10px] font-bold text-muted uppercase tracking-widest mb-3 border-l-2 border-emerald-500 pl-2">1. Tamanho</h3>
              <div className="grid grid-cols-1 gap-2">
                {acaiBases.map(b => (
                  <button key={b.id} onClick={() => setBuilder({...builder, base: b, standard: b.ingredients ? b.ingredients.split(',').map(i=>i.trim()) : []})}
                    className={`p-3 rounded-lg border transition-all text-left ${builder.base?.id === b.id ? 'bg-emerald-600/10 border-emerald-500' : 'bg-surface-light border-border hover:border-border'}`}>
            ...
            {builder.standard && builder.standard.length > 0 && (
              <section className={!builder.base ? 'opacity-20 pointer-events-none' : ''}>
                <h3 className="text-[10px] font-bold text-muted uppercase tracking-widest mb-3 border-l-2 border-red-500 pl-2">2. Retiradas</h3>
                <div className="flex flex-wrap gap-2">
                  {(builder.standard || []).map(ing => (
                    <button key={ing} onClick={() => toggleRemoval(ing)}
                      className={`px-3 py-1.5 rounded-md border text-[9px] font-bold transition-all ${builder.removals.includes(ing) ? 'bg-red-500/10 border-red-500 text-red-500' : 'bg-surface-light border-border text-muted'}`}>
            ...
            <section className={!builder.base ? 'opacity-20 pointer-events-none' : ''}>
              <h3 className="text-[10px] font-bold text-muted uppercase tracking-widest mb-3 border-l-2 border-orange-500 pl-2">4. Notas e Ajuste</h3>
          ...
          <div className={`flex-1 flex flex-col border-l border-border pl-6 ${!builder.base ? 'opacity-20 pointer-events-none' : ''}`}>
            <h3 className="text-[10px] font-bold text-muted uppercase tracking-widest mb-4 border-l-2 border-purple-500 pl-2">3. Adicionais Extras</h3>
            <div className="flex-1 overflow-y-auto grid grid-cols-2 xl:grid-cols-3 gap-3 pr-2 custom-scrollbar">
              {availableAddons.map(a => {
                ...
                return (
                  <div key={a.id} className={`p-3 rounded-xl border transition-all flex items-center justify-between ${qty > 0 ? 'bg-purple-600/10 border-purple-500' : 'bg-surface-light border-border'}`}>
                    <div className="flex flex-col min-w-0"><span className="font-bold text-[10px] uppercase text-primary truncate">{a.name}</span><span className="font-mono text-emerald-500 text-[9px]">R${(a.price || 0).toFixed(2)}</span></div>
                    <div className="flex items-center gap-2 bg-surface-light p-1 rounded-lg border border-border">
                      ...
                      <span className={`text-xs font-bold w-4 text-center ${qty > 0 ? 'text-purple-400' : 'text-muted'}`}>{qty}</span>
                      <button onClick={() => updateExtraInBuilder(a, 1)} className="w-6 h-6 bg-surface-light rounded flex items-center justify-center hover:bg-border">+</button>
                    </div>
                  </div>
                )
        ...
        <div className="p-4 bg-surface-light border-t border-border flex items-center justify-between px-8">
          ...
          <div className="flex items-center gap-8">
            ...
            <button disabled={!builder.base} onClick={confirmFullBuild}
              className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-surface-light text-slate-950 font-bold py-3 px-8 rounded-xl transition-all active:scale-95">LANÇAR</button>
          </div>
        </div>
      </div>
    </div>
