          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => { setAdvancedReportData(null); loadReports(); }}
                className={`text-[10px] font-bold uppercase px-3 py-1 rounded transition-all ${!advancedReportData ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600 hover:text-gray-900'}`}>Hoje</button>
              <button onClick={() => { setAdvancedReportData(null); }}
                className={`text-[10px] font-bold uppercase px-3 py-1 rounded transition-all ${advancedReportData ? 'bg-emerald-600 text-white' : 'bg-gray-200 text-gray-600 hover:text-gray-900'}`}>Período</button>
            </div>
          </div>
            ...
            <div className="bg-gray-100 p-4 rounded-xl border border-gray-300 space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setCashMove({...cashMove, type: 'ENTRADA'})}
                  className={`py-2 rounded font-bold text-[10px] flex items-center justify-center gap-1 transition-all ${cashMove.type === 'ENTRADA' ? 'bg-emerald-600/20 text-emerald-500 border border-emerald-500/50' : 'bg-gray-200 text-gray-600 border border-gray-300'}`}>
                  <ArrowUpCircle size={14}/> ENTRADA</button>
                <button onClick={() => setCashMove({...cashMove, type: 'SAIDA'})}
                  className={`py-2 rounded font-bold text-[10px] flex items-center justify-center gap-1 transition-all ${cashMove.type === 'SAIDA' ? 'bg-red-600/20 text-red-500 border border-red-500/50' : 'bg-gray-200 text-gray-600 border border-gray-300'}`}>
